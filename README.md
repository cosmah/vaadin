...
`java -jar target/vaadin-1.0-SNAPSHOT.jar`
...
```
docker run -p 8080:8080 vaadin:latest
```
...
If the pods say `Container image "vaadin:latest" is not present with pull policy of Never` then you have not built your application using Docker or there is a mismatch in the name. Use `docker images ls` to see which images are available.
